...
def init_git_repo():
    ...
    commands = [
        ...
        "git config --local core.autocrlf false",  # 解决换行符问题
...
def git_pull():
    """从GitHub拉取更改"""
    print("\n从 GitHub 拉取更新...")
    result = run_command("git pull origin master --no-rebase")
    if result:
        print("✓ 拉取成功")
    else:
        print("✗ 拉取失败，请检查网络连接")
...
def show_menu():
    ...
    print("=" * 60)
    print("1. 查看状态 (status)")
    print("2. 提交并推送代码 (commit & push)")
    print("3. 从 GitHub 拉取更新 (pull)")
    print("4. 一键同步 (pull + push)")
    print("5. 初始化 Git 仓库 (首次使用)")
    print("0. 退出")
    print("=" * 60)
...
def interactive_mode():
    ...
    while True:
        ...
        if choice == "1":
            ...
            message = input("\n输入提交信息 (默认: Update project): ").strip()
            if not message:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
使用示例:
  python sync_github.py          # 进入交互模式
  python sync_github.py --push   # 直接推送
  python sync_github.py --pull   # 直接拉取
  python sync_github.py --sync   # 一键同步
  python sync_github.py --status # 查看状态
  python sync_github.py --init   # 初始化仓库
        """
    )

    parser.add_argument("--push", action="store_true", help="直接推送到 GitHub")
    parser.add_argument("--pull", action="store_true", help="直接从 GitHub 拉取")
    parser.add_argument("--sync", action="store_true", help="一键同步（拉取+推送）")
    ...
    print(f"GitHub 仓库: https://github.com/frankwu510/stock-data-updater")
